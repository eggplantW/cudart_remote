...
#define GC_PTX_PATH "."
#define GC_RemoteExecPath "/home/run/cuda-workplace/cudart_remote_pipeline/src/RemoteAssistant"
...
struct StreamAttr 
{
	int streamTag;
	int deviceId;
	std::map<int,MPI_Request*>lastReq;
...
extern "C" 
{
	...
	//printf("cudaStreamSynchronize2:pid = %x,threadId = %d,currentDevice = %d,stream = %x\n",pid,threadId,device,stream);
		std::map<int,MPI_Request*>::iterator mpiReqIt;
		for(mpiReqIt = streamIt->second.lastReq.begin();mpiReqIt != streamIt->second.lastReq.end();++mpiReqIt)
		{
			//printf("cudaStreamSync before MPI_Wait:threadId = %d\n",threadId);
			MPI_Wait(mpiReqIt->second,MPI_STATUS_IGNORE);
			//printf("cudaStreamSync after MPI_Wait:threadId = %d\n",threadId);
		}
		pthread_mutex_unlock(&GC_StreamTagMutex);
	...
	__host__ __cudart_builtin__ cudaError_t CUDARTAPI cudaMemcpyAsync(void *dst, const void *src, size_t count, enum cudaMemcpyKind kind, cudaStream_t stream) 
	{
		...
		MPI_Request *req;
		std::map<int,MPI_Request*>::iterator mpiReqIt = streamIt->second.lastReq.find(threadId);
		if(mpiReqIt == streamIt->second.lastReq.end())
		{
			req = (MPI_Request*)malloc(sizeof(MPI_Request));
			*req = MPI_REQUEST_NULL;
			streamIt->second.lastReq[threadId] = req;
		}
		else
			req = mpiReqIt->second;
		pthread_mutex_unlock(&GC_StreamTagMutex);
		...
		{		
			...
			_dst = (void *)(dstP & 0x0000ffffffffffff);
			CudaMemcpyAsyncMsg_t msg(_dst, count, kind, stream, threadId);
			mpi_error( MPI_Send(&msg, sizeof(msg), MPI_BYTE, GC_DeviceList[device], NullTag, GC_DeviceComm) );
			//printf("memcpy host to src=%lx device=%d count=%lx,tag=%lx\n",_src,GC_DeviceList[device],count,threadId<<16|streamTag);	
			while(_count > HOST_BUFFER_INIT_SIZE)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Isend(_src,HOST_BUFFER_INIT_SIZE,MPI_BYTE,GC_DeviceList[device],threadId<<16|streamTag,GC_DeviceComm,req));
				_count = _count - HOST_BUFFER_INIT_SIZE;
				_src = _src + HOST_BUFFER_INIT_SIZE;
			}
			if(_count > 0)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Isend(_src,_count,MPI_BYTE,GC_DeviceList[device],threadId<<16|streamTag,GC_DeviceComm,req));
			}			
			pthread_mutex_lock(&GC_StreamTagMutex);
			streamIt->second.lastReq[threadId] = req;
			pthread_mutex_unlock(&GC_StreamTagMutex);
			...
			while(_count > HOST_BUFFER_INIT_SIZE)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Irecv(_dst,HOST_BUFFER_INIT_SIZE,MPI_BYTE,GC_DeviceList[device],threadId<<16|streamTag,GC_DeviceComm,req));
				_count = _count - HOST_BUFFER_INIT_SIZE;
				_dst = _dst + HOST_BUFFER_INIT_SIZE;
			}
			if(_count > 0)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Irecv(_dst,_count,MPI_BYTE,GC_DeviceList[device],threadId<<16|streamTag,GC_DeviceComm,req));
			}	
			pthread_mutex_lock(&GC_StreamTagMutex);
			streamIt->second.lastReq[threadId] = req;
			pthread_mutex_unlock(&GC_StreamTagMutex);
		...
		{	
			...
			while(_count > HOST_BUFFER_INIT_SIZE)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Irecv(tmpDst,HOST_BUFFER_INIT_SIZE,MPI_BYTE,GC_DeviceList[srcDevice],threadId<<16|1,GC_DeviceComm,req));
				_count = _count - HOST_BUFFER_INIT_SIZE;
				tmpDst = tmpDst + HOST_BUFFER_INIT_SIZE;
			}
			if(_count > 0)
			{
				if(*req != MPI_REQUEST_NULL) 
					mpi_error(MPI_Request_free(req));
				mpi_error(MPI_Irecv(tmpDst,_count,MPI_BYTE,GC_DeviceList[srcDevice],threadId<<16|1,GC_DeviceComm,req));
			}	
